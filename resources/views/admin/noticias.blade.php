...
<div style="max-width:1100px;margin:0 auto;padding:36px 24px 48px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:32px;flex-wrap:wrap;gap:16px;">
        ...
        <a href="{{ route('admin.noticias.create') }}"
           style="display:inline-flex;align-items:center;gap:8px;background:#1565c0;color:#fff;font-weight:700;font-size:0.95rem;padding:11px 22px;border-radius:10px;text-decoration:none;box-shadow:0 2px 8px rgba(21,101,192,0.25);transition:background 0.2s;"
    ...
    <div style="background:#fff;border-radius:16px;box-shadow:0 1px 8px rgba(0,0,0,0.07);padding:64px 32px;text-align:center;">
        ...
        <a href="{{ route('admin.noticias.create') }}" style="display:inline-block;margin-top:18px;background:#1565c0;color:#fff;font-weight:600;padding:10px 24px;border-radius:8px;text-decoration:none;font-size:0.95rem;">Criar primeira notícia</a>
    </div>
    @else
            ...
            <div style="flex:1;min-width:0;">
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px;">
                    ...
                    @if($noticia->publicada)
                        <span style="background:#dcfce7;color:#166534;font-size:0.72rem;font-weight:700;padding:2px 10px;border-radius:99px;letter-spacing:0.04em;">PUBLICADA</span>
                    @else
                    ...
                    @if($noticia->institucional)
                        <span style="background:#eff6ff;color:#1d4ed8;font-size:0.72rem;font-weight:700;padding:2px 10px;border-radius:99px;letter-spacing:0.04em;">INSTITUCIONAL</span>
                    @endif
                </div>
                <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
                    ...
                    <a href="{{ asset('storage/' . $noticia->imagem) }}" target="_blank"
                       style="font-size:0.85rem;color:#0891b2;display:flex;align-items:center;gap:5px;text-decoration:none;font-weight:600;">
            ...
            <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;flex-wrap:wrap;">
                ...
                <form method="POST" action="{{ route('admin.noticias.toggle-publicar', $noticia->id) }}" style="display:inline;">
                    ...
                    <button type="submit" title="{{ $noticia->publicada ? 'Despublicar' : 'Publicar' }}"
                        style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;border:1px solid {{ $noticia->publicada ? '#fde68a' : '#bbf7d0' }};background:{{ $noticia->publicada ? '#fefce8' : '#f0fdf4' }};color:{{ $noticia->publicada ? '#92400e' : '#166534' }};cursor:pointer;transition:background 0.15s;"
                ...
                <a href="{{ route('admin.noticias.edit', $noticia->id) }}"
                   title="Editar"
                   style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:#eff6ff;color:#1d4ed8;text-decoration:none;border:1px solid #bfdbfe;transition:background 0.15s;"
