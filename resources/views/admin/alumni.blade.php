    ...
    @else
        <div style="display:grid;gap:14px;">
            ...
            <div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:20px 24px;display:flex;align-items:flex-start;gap:18px;">
                ...
                {{-- Avatar --}}
                <div style="width:46px;height:46px;border-radius:50%;background:#e3f2fd;color:#1565c0;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-weight:700;font-size:1.1rem;">
                ...
                <div style="flex:1;min-width:0;">
                    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;">
                        <div>
                            ...
                            <div style="font-size:0.84rem;color:#64748b;margin-top:3px;">
                                {{ $alumnus->curso }}
                                @if($alumnus->ano)
                                    &nbsp;&middot;&nbsp;<span style="background:#e3f2fd;color:#1565c0;padding:1px 8px;border-radius:20px;font-size:0.77rem;font-weight:600;">{{ $alumnus->ano }}</span>
                                @endif
                            </div>
                        </div>
                        {{-- Status badges --}}
                    ...
                    @if($hasDepoimento)
                        <details style="margin-top:10px;">
                            <summary style="cursor:pointer;color:#1565c0;font-size:0.82rem;font-weight:600;user-select:none;list-style:none;display:inline-flex;align-items:center;gap:5px;">
                            ...
                            <p style="margin-top:8px;font-size:0.85rem;color:#475569;line-height:1.5;border-left:3px solid #1565c0;padding-left:10px;">"{{ $satisfacao }}"</p>
                        </details>
                    @endif
                </div>
                ...
                <div style="display:flex;flex-direction:column;gap:8px;flex-shrink:0;">
                    <form method="POST" action="{{ route('admin.alumni.toggle-publicar', $alumnus->id) }}">
                        @csrf
                        <button type="submit"
                            style="padding:7px 14px;border-radius:8px;font-size:0.8rem;font-weight:600;border:none;cursor:pointer;white-space:nowrap;{{ $alumnus->publicado ? 'background:#fff8e1;color:#f57f17;' : 'background:#1565c0;color:#fff;' }}"
                    ...
                    @if($alumnus->trabalha)
                        <form method="POST" action="{{ route('admin.alumni.toggle-testemunho', $alumnus->id) }}">
                            @csrf
                            <button type="submit"
                                style="padding:7px 14px;border-radius:8px;font-size:0.8rem;font-weight:600;border:none;cursor:pointer;white-space:nowrap;{{ $alumnus->testemunho ? 'background:#f3e5f5;color:#7b1fa2;' : 'background:#e3f2fd;color:#1565c0;' }}"
                                onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">
