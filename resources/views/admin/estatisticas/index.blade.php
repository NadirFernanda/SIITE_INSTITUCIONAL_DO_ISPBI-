...
<div style="max-width:1000px;margin:0 auto;padding:36px 24px 48px;">
    ...
    <div style="background:#fff;border-radius:16px;box-shadow:0 1px 8px rgba(0,0,0,0.07);padding:64px 32px;text-align:center;">
        ...
        <a href="{{ route('admin.estatisticas.create') }}" style="display:inline-block;margin-top:18px;background:#1565c0;color:#fff;font-weight:600;padding:10px 24px;border-radius:8px;text-decoration:none;font-size:0.95rem;">Criar primeira estatística</a>
    </div>
    @else
    <div style="display:grid;gap:12px;">
        ...
        <div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;box-shadow:0 1px 6px rgba(0,0,0,0.06);padding:18px 22px;display:flex;align-items:center;gap:18px;"
            ...
            {{-- Ordem badge --}}
            <div style="flex-shrink:0;width:40px;height:40px;border-radius:10px;background:#eff6ff;border:1px solid #bfdbfe;display:flex;align-items:center;justify-content:center;font-size:0.8rem;font-weight:800;color:#1d4ed8;">
                {{ $estatistica->ordem ?? '—' }}
            </div>
            ...
            <div style="flex:1;min-width:0;">
                <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;margin-bottom:3px;">
                    ...
                    <span style="font-size:1.1rem;font-weight:800;color:#1565c0;">{{ $estatistica->valor }}</span>
                </div>
            ...
            <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
                ...
                <form action="{{ route('admin.estatisticas.destroy', $estatistica) }}" method="POST" style="display:inline;"
                      onsubmit="return confirm('Confirma que deseja excluir «{{ addslashes($estatistica->titulo) }}»?')">
                    @csrf
                    @method('DELETE')
