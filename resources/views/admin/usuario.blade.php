...
<div style="padding:32px 24px;max-width:820px;margin:0 auto;">
    <a href="{{ route('admin.usuarios') }}" style="display:inline-block;margin-bottom:18px;color:#1565c0;">&larr; Voltar à lista de utilizadores</a>

    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:22px;">
        <div style="display:flex;gap:18px;align-items:center;">
            @php
            $avatarBg  = match($usuario->role) { 'admin'=>'#e3f2fd','tecnico'=>'#dcfce7','daac'=>'#ede9fe','secretaria'=>'#fdf4ff','subcomissao_correcao'=>'#f5f3ff','subcomissao_lancamento'=>'#d1fae5','presidencia'=>'#faf5ff', default=>'#f1f5f9' };
            $avatarClr = match($usuario->role) { 'admin'=>'#1565c0','tecnico'=>'#15803d','daac'=>'#7c3aed','secretaria'=>'#a21caf','subcomissao_correcao'=>'#6d28d9','subcomissao_lancamento'=>'#0f766e','presidencia'=>'#7c3aed', default=>'#64748b' };
            @endphp
        ...
        <div style="margin-top:18px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                ...
                <form method="POST" action="{{ route('admin.usuarios.password', $usuario) }}" autocomplete="off">
                    ...
                    <div style="display:flex;flex-direction:column;gap:8px;">
                        ...
                        <div style="display:flex;gap:8px;margin-top:6px;">
                            <button type="submit" style="background:#f59e0b;color:#fff;border:none;border-radius:8px;padding:8px 14px;font-weight:700;">Guardar</button>
            ...
            <div style="background:#f5f3ff;border:1px solid #c4b5fd;border-radius:10px;padding:14px;">
                <div style="font-size:0.88rem;font-weight:700;color:#6d28d9;margin-bottom:10px;">Assinatura digitalizada</div>
                ...
                @else
                    <p style="margin:0 0 12px;color:#6d28d9;">Nenhuma assinatura guardada. Carregue uma imagem PNG/JPG da assinatura manuscrita em papel branco.</p>
                @endif
                ...
                <form method="POST" action="{{ route('admin.usuarios.assinatura', $usuario) }}" enctype="multipart/form-data" style="margin-top:10px;display:flex;gap:8px;align-items:center;">
                    ...
                    <button type="submit" style="background:#7c3aed;color:#fff;border:none;border-radius:8px;padding:8px 12px;">Guardar</button>
                </form>
            </div>
        </div>
