...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px; flex-wrap: wrap; gap: 16px;">
	<h1 style="font-size: 2.6rem; color: #1e3a5f; font-weight: 800; letter-spacing: -1px; margin: 0;">Mídia</h1>
	<form action="/admin/midia" method="POST" enctype="multipart/form-data" style="display: flex; align-items: center; gap: 10px;">
		...
		<button type="submit" style="background: #1e3a5f; color: #fff; padding: 12px 28px; border: none; border-radius: 8px; font-weight: 700; font-size: 1.1rem; box-shadow: 0 2px 8px rgba(21,101,192,0.10); transition: background 0.2s; outline: none;">Nova Mídia</button>
	</form>
</div>
		...
		@forelse($midias as $midia)
			<tr style="border-bottom:1px solid #eaeff5;">
				...
				<td style="padding:12px 18px;">
					...
					@else
						<a href="{{ asset('storage/' . $midia->caminho) }}" target="_blank" style="color:#1e3a5f; font-weight:500; text-decoration:underline;">Ver arquivo</a>
					@endif
				</td>
