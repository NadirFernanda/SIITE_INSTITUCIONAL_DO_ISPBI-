...
<head>
    ...
    <title>Painel Lançamento — ISP-Bié</title>
    <style>
        body { margin:0; font-family:'Segoe UI','Arial',sans-serif; background:#f8fafc; }
        .sidebar {
            width:220px; background:#0f766e; color:#fff; height:100vh;
            position:fixed; top:0; left:0; padding-top:20px;
            box-shadow:2px 0 16px rgba(15,118,110,0.15);
        ...
        .sidebar-logo span { display:block; font-size:0.78rem; font-weight:400; color:#5eead4; margin-top:2px; }
        .sidebar a {
        ...
        .sidebar a:hover, .sidebar a.active { background:#115e59; }
        .main-content { margin-left:220px; padding:36px 28px; min-height:100vh; }
        .header {
            background:#0f766e; padding:18px 36px; margin-left:220px;
...
<body>
    <div class="sidebar">
        <div class="sidebar-logo">
            ISP-Bié
            <span>Painel Lançamento</span>
        </div>
        <div style="flex:1;">
            <a href="{{ route('lancamento.salas.index') }}"
               class="{{ request()->routeIs('lancamento.salas.*') ? 'active' : '' }}">
        ...
        <div style="margin:16px 12px;">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                        style="width:100%;background:#fff;color:#0f766e;font-weight:700;padding:9px 0;border:none;border-radius:8px;cursor:pointer;">
                    Sair
                </button>
            </form>
        </div>
    ...
    <div class="header">
        Instituto Superior Politécnico do Bié — Equipa de Lançamento
    </div>
