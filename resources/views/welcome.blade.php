...
<section class="py-4 bg-white border-b border-gray-200">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <!-- Barra azul com redes sociais e busca -->
    <div x-data="{ dark: false }" :class="dark ? 'bg-[#1a237e]' : 'bg-[#2563eb]'" class="transition-colors duration-500">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-wrap items-center justify-between gap-3 py-2.5">

        {{-- Lado esquerdo: SIGA-NOS + ícones --}}
        <div class="flex items-center gap-3">
          <span class="text-white font-semibold text-xs tracking-widest uppercase opacity-80 hidden sm:inline">Siga-nos</span>
          <div class="flex items-center gap-1.5">
            <a href="https://www.facebook.com/search/top?q=instituto%20superior%20polit%C3%A9cnico%20do%20bi%C3%A9" target="_blank" rel="noopener" aria-label="Facebook"
               class="flex items-center justify-center w-8 h-8 rounded-full bg-white/15 hover:bg-white hover:text-[#2563eb] text-white transition-all duration-200">
        ...
        {{-- Lado direito: busca + modo escuro --}}
        <div class="flex items-center gap-2">
          <form action="/busca" method="GET" class="flex items-center bg-white/15 hover:bg-white/20 rounded-full border border-white/30 focus-within:bg-white/25 focus-within:border-white/60 transition-all duration-200 overflow-hidden">
            <input type="text" name="q" placeholder="Busca institucional..." aria-label="Pesquisar"
                   class="bg-transparent text-white placeholder-white/70 text-xs px-3 py-1.5 focus:outline-none w-40 sm:w-52" autocomplete="off">
            <button type="submit" aria-label="Buscar" class="flex items-center justify-center w-7 h-7 mr-0.5 rounded-full bg-[#F05A28] hover:bg-[#d44d20] text-white transition-colors duration-200 flex-shrink-0">
          ...
          <button @click="dark = !dark" type="button" aria-label="Alternar modo escuro"
                  class="flex items-center gap-1.5 text-white/80 hover:text-white text-xs font-medium px-2.5 py-1.5 rounded-full border border-white/25 hover:border-white/60 bg-white/10 hover:bg-white/20 transition-all duration-200 whitespace-nowrap">
            <svg x-show="!dark" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1111.21 3a7 7 0 109.79 9.79z"/></svg>
            <svg x-show="dark" x-cloak class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v2m0 14v2m9-9h-2M5 12H3m15.364-6.364l-1.414 1.414M6.05 17.95l-1.414 1.414M17.95 17.95l-1.414-1.414M6.05 6.05L4.636 7.464"/></svg>
            <span x-text="dark ? 'Modo claro' : 'Modo escuro'" class="hidden sm:inline"></span>
          </button>
        </div>
