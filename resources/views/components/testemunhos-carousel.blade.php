...
@endphp
<div class="relative w-full max-w-5xl mx-auto my-12" x-data="{ current: 0, total: {{ $testemunhos->count() }} }" x-init="setInterval(() => { current = (current + 1) % total }, 7000)">
    <div class="overflow-hidden rounded-2xl shadow-lg bg-white">
        <div class="flex transition-transform duration-700" :style="'transform: translateX(-' + (current * 100) + '%)'">
            @foreach($testemunhos as $t)
                <div class="min-w-full flex flex-col justify-between p-8">
