...
@if($totalSlides > 0)
<div class="relative" x-data="{ currentSlide: 0 }" x-init="if ({{ $totalSlides }} > 1) { setInterval(() => { currentSlide = (currentSlide + 1) % {{ $totalSlides }} }, 5000) }">
    <div class="relative h-[300px] sm:h-[400px] lg:h-[500px] overflow-hidden">
        ...
        @endforeach
        <!-- Setas de navegação -->
        <button @click="currentSlide = (currentSlide + {{ $totalSlides }} - 1) % {{ $totalSlides }}" class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full p-2 shadow focus:outline-none z-20" x-show="{{ $totalSlides }} > 1">
            <svg class="w-6 h-6 text-[#2563eb]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
        </button>
        <button @click="currentSlide = (currentSlide + 1) % {{ $totalSlides }}" class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full p-2 shadow focus:outline-none z-20" x-show="{{ $totalSlides }} > 1">
            <svg class="w-6 h-6 text-[#2563eb]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
        </button>
        <!-- Indicadores do carrossel -->
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2" x-show="{{ $totalSlides }} > 1">
...
@else
    ...
    <div class="relative h-[300px] sm:h-[400px] lg:h-[500px] flex items-center justify-center bg-gray-100">
